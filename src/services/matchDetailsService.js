 ...
 * @param {string} [tab='overview'] - The tab to fetch data from
 * @returns {Object} Detailed match information including maps, scores, and round data
 */
async function getMatchDetails(matchId, tab = 'overview') {
    if (!matchId) throw new Error("Match ID is required");
  
    try {
      console.log(`Fetching match details for ${matchId} with tab: ${tab}...`);
      
      const $ = await request({
        uri: `${vlrgg_url}/${matchId}?tab=${tab}`,
        transform: (body) => cheerio.load(body),
      });

      // If tab is economy, only return economy data
      if (tab === 'economy') {
        const economyData = extractEconomyData($);
        return {
          matchId,
          tab,
          ...economyData
      ...
      // For other tabs, return full match info
      // Detect match status
  ...
  console.log(`Processed ${matchInfo.playerStats.length} player statistics`);
      ...
      // Add new economy data extraction if tab is economy
      if (tab === 'economy') {
          matchInfo.economy = extractEconomyData($);
        }
...
// New function to extract economy data
function extractEconomyData($) {
  ...
  // Extract team economy data from the table
  let validTeams = 0;
  $(".wf-table-inset.mod-econ tbody tr").each((index, element) => {
    if (index === 0) return; // Skip header row
    
    const teamName = $(element).find('.team').text().trim();
    // Skip empty or invalid team names
    if (!teamName || teamName.includes('\t') || validTeams >= 2) return;
    
    const teamData = {
      name: teamName,
      logo: $(element).find('.team img').attr('src'),
      pistolWon: parseInt($(element).find('td:nth-child(2) .stats-sq').text().trim()) || 0,
      eco: {
        total: parseInt($(element).find('td:nth-child(3) .stats-sq').text().trim().split('(')[0]) || 0,
        won: parseInt($(element).find('td:nth-child(3) .stats-sq').text().trim().match(/\((\d+)\)/) ? $(element).find('td:nth-child(3) .stats-sq').text().trim().match(/\((\d+)\)/)[1] : 0) || 0
      },
      semiEco: {
        total: parseInt($(element).find('td:nth-child(4) .stats-sq').text().trim().split('(')[0]) || 0,
        won: parseInt($(element).find('td:nth-child(4) .stats-sq').text().trim().match(/\((\d+)\)/) ? $(element).find('td:nth-child(4) .stats-sq').text().trim().match(/\((\d+)\)/)[1] : 0) || 0
      },
      semiBuy: {
        total: parseInt($(element).find('td:nth-child(5) .stats-sq').text().trim().split('(')[0]) || 0,
        won: parseInt($(element).find('td:nth-child(5) .stats-sq').text().trim().match(/\((\d+)\)/) ? $(element).find('td:nth-child(5) .stats-sq').text().trim().match(/\((\d+)\)/)[1] : 0) || 0
      },
      fullBuy: {
        total: parseInt($(element).find('td:nth-child(6) .stats-sq').text().trim().split('(')[0]) || 0,
        won: parseInt($(element).find('td:nth-child(6) .stats-sq').text().trim().match(/\((\d+)\)/) ? $(element).find('td:nth-child(6) .stats-sq').text().trim().match(/\((\d+)\)/)[1] : 0) || 0
      }
    };
    
    economyData.teams.push(teamData);
    validTeams++;
  });
...
module.exports = { getMatchDetails, extractEconomyData };
